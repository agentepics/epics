...
1. Read `state.json` for the current approval policy and exception list.
2. Review active Epics against the policy.
3. Route work into automatic, ask-first, or blocked states.
4. When approval is needed, ask and record the decision in `log/`.
5. Update `state.json` with approval outcomes and any policy changes.

## Rules

- Approval policy must be explicit in `state.json` before autonomous work proceeds.
