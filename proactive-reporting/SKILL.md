...
1. Read `state.json` for reporting cadence, scope, and per-Epic duties.
...
5. Write a log entry to `log/` and update `state.json` with delivery status.

## Rules

- Reporting frequency must follow the user's preference in `state.json`.
